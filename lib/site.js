...
class Site {
  ...
  use (/* [name], conf|plugin|pluginbuilder, [opts] */) {
    ...
    if (confirmedPlugin) {
      ...
      site._plugins.use(new AwooPlugin(pluginName, plugin))
      return site
    }
...
/**
 * This little class server as wrapper around function
 * Basically it help us to name anonymous function
 */
class AwooPlugin {
  constructor (name, fn) {
    this.name = name
  ...
  run() {
    return this.fn.apply(this.fn, arguments)
  }

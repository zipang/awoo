...
const _DEFAULTS = {
  ...
  debug: console.log,
  dry_run: false,
  content_files: ['.md', '.markdown', '.yaml'], // load only the content of these files
...
class Site {
  ...
  async build() {
    let { _config, _readFiles, _plugins, _writeFiles } = this

    let buildChain = chain('build', {
      // give every plugin a link to the site's configuration and a free logger function
      inject: [_config, _config.debug]
    }, [_readFiles, _plugins, _writeFiles])

    let files = await buildChain.run()
    return this
  }
}
...
class AwooPlugin {
  ...
  constructor (name, fn, debug) {
    ...
    this.debug = debug
    if (debug) debug(`Added plugin ${this}`)
  }
